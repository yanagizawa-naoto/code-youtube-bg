...
Use adaptive readability mode, which keeps the older whole-window opacity as the baseline, then lets empty editor space show more video:
...
- `CODE_YOUTUBE_BG_ADAPTIVE=1` keeps the legacy whole-window opacity as a baseline while empty areas show more video.
